...
export const importTavernCharacter = (file: File): Promise<Character> => {
  ...
  async function processCharacterData(characterData: any): Promise<Character> {
    ...
    if (characterData.post_history_instructions) {
      construct.post_history_instructions =
        characterData.post_history_instructions.replaceAll("\r", "");
    }
    ...
    if (
      ...
      characterData.alternate_greetings.length > 0 &&
    ...
    if (
      characterData.thought_pattern &&
      characterData.thought_pattern.trim().length > 0
    ) {
      construct.thought_pattern = characterData.thought_pattern.replaceAll(
        "\r",
        ""
      );
    }
    ...
    if (
      ...
      characterData.visual_description.trim().length > 0
    ) {
      construct.visual_description = characterData.visual_description.replaceAll(
        "\r",
        ""
      );
    }
    ...
    if (
      ...
      characterData.system_prompt.trim().length > 0
    ) {
      ...
      reader.onload = async (event) => {
        if (event.target && event.target.result instanceof ArrayBuffer) {
          ...
          const textChunks = chunks
            .filter((chunk: Chunk) => chunk.name === "tEXt")
            .map((chunk: Chunk) => {
