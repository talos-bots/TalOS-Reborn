...
const lastIntentData: any = null;
let showTyping = false;
let delay = 0;
let doDelay = false;
const lurkingChannels: string[] = [];
...
export const addDiffusionWhitelist = (channelID: string): void => {
    const whitelist = getDiffusionWhitelist();
...
export async function getUsername(userID: string, channelID: string){
    ...
    try {
        const user = await disClient.users.fetch(userID);
        const name = user?.displayName !== undefined ? user.displayName : user.username;
        console.log(name);
        return name;
    } catch (error) {
...
export async function handleDiscordMessage(message: Message) {
    ...
    const chatLogData = await getChat(message.channel.id);
    let chatLog;
    if (chatLogData) {
    ...
    for (let i = 0; i < chatLog.constructs.length; i++) {
        const constructDoc = await getConstruct(chatLog.constructs[i]);
        const construct = assembleConstructFromData(constructDoc);
    ...
    if(isMultiConstructMode() && !message.channel.isDMBased()){
        const lastMessageContent = chatLog.lastMessage.text;
        const shuffledConstructs = constructArray;
        ...
        const mentionedConstruct = containsName(lastMessageContent, constructArray);
        if (mentionedConstruct) {
            const mentionedIndex = shuffledConstructs.findIndex(construct => construct.name === mentionedConstruct);
        ...
        const lastMessageText = chatLog?.lastMessage?.text;
        let iterations = 0;
        ...
        while (true && !generationFailure) {
            ...
            for(let i = 0; i < shuffledConstructs.length; i++) {
                ...
                const config = shuffledConstructs[i].defaultConfig;
                
                if (chatLog?.lastMessage?.isHuman) { // Last message is from a human
                    if (config.replyToUser >= Math.random()) {
                        const replyLog = await doCharacterReply(shuffledConstructs[i], chatLog, message);
        ...
        if(!config.doLurk){
            ...
            const wasMentioned = isMentioned(chatLog.lastMessage.text, constructArray[0]) && chatLog.lastMessage.isHuman;
            if(wasMentioned){
                if(config.replyToUserMention >= Math.random()){
                    ...
                    const replyLog = await doCharacterReply(constructArray[0], chatLog, message);
                    if(replyLog !== undefined){
                        chatLog = replyLog;
                    }
                }
...
async function doCharacterReply(construct: ConstructInterface, chatLog: ChatInterface, message: Message | CommandInteraction){
    ...
    const primaryConstruct = retrieveConstructs()[0];
    ...
    const alias = await getUsername(authorID, chatLog._id);
    ...
    if(construct.defaultConfig.haveThoughts && construct.defaultConfig.thinkBeforeChat){
        if(construct.defaultConfig.thoughtChance >= Math.random()){
            ...
            const thoughtChatLog = await doCharacterThoughts(construct, chatLog, message);
    ...
    for(let i = 0; i < constructArray.length; i++){
        ...
        if(wasMentionedByHuman){
            if(config.replyToUserMention >= Math.random()){
                ...
                const replyLog = await doCharacterReply(constructArray[i], chatLog, message);
                if(replyLog !== undefined){
                    chatLog = replyLog;
                }
            }
...
export async function continueChatLog(interaction: CommandInteraction) {
    ...
    if(!registered) return;
    const constructArray = [];
    const chatLogData = await getChat(interaction.channelId).then((doc) => {
        return doc;
    }).catch((err) => {
        console.log(err);
    ...
    if(isMultiConstructMode() && !interaction.channel.isDMBased()){
        ...
        while (true) { // The loop to make replies continuously until no construct feels the need to reply
            ...
            for(let i = 0; i < shuffledConstructs.length; i++) {
                ...
                if (chatLog?.lastMessage?.isHuman) { // Last message is from a human
                    if (config.replyToUser >= Math.random()) {
                        const replyLog = await doCharacterReply(shuffledConstructs[i], chatLog, interaction);
                        if (replyLog !== undefined) {
                            chatLog = replyLog;
                        }
                        shouldContinue = true;
        ...
        if(!config.doLurk === true){
            ...
            if(wasMentioned){
                if(config.replyToUserMention >= Math.random()){
                    ...
                    const replyLog = await doCharacterReply(constructArray[0], chatLog, interaction);
                    if(replyLog !== undefined){
                        chatLog = replyLog;
                    }
                }
...
export async function handleRengenerateMessage(message: Message){
    ...
    const edittedMessage = await regenerateMessageFromChatLog(chatLog, message.content);
...
export async function assembleUserProfile(ServerID: string, userID: string, isDM = false){
    if(!isDM){
        ...
        const userString = `[{{char}} is speaking to ${userObject.username} ${userObject.nickname !== 'No nickname' ? `(${userObject.nickname})` : ''} in ${serverData.name}. ${userObject.username} is ${userObject.status}. ${userObject.username} joined the server on ${userObject.joined}. ${userObject.username} created their account on ${userObject.created}. ${userObject.username} has the following roles: ${userObject.roles.join(', ')}.]`;
        return userString;
    }else{
        return null;
    }
...
export async function doImageReaction(message: Message){
    ...
    const imageData = await makeImage(prompt).then((data) => {
        return data;
    }).catch((err) => {
        console.log(err);
        return null;
...
export async function removeConstructFromChatLog(constructID: string, chatLogID: string){
    ...
    const currentLog = assembleChatFromData(currentLogData);
    if(currentLog === null){
        return;
    }
