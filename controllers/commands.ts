...
export const RegisterCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        const constructArray: CharacterInterface[] = [];
        for(let i = 0; i < constructs.length; i++){
            const assembledConstruct = assembleConstructFromData(constructs[i]);
...
export const ListCharactersCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        for (let i = 0; i < constructs.length; i++) {
            const constructDoc = await getConstruct(constructs[i]);
...
export const ClearLogCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        const pulledChat = await getChat(interaction.channelId) as ChatInterface;
        if(pulledChat === null){
            await interaction.editReply({
...
export const DoCharacterGreetingsCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        const construct = assembleConstructFromData(constructDoc);
        const user = getUsername(interaction.user.id, interaction.channelId);
        if(construct === null) return;
        const randomGreeting = construct?.greetings[Math.floor(Math.random() * construct.greetings.length)];
        ...
        const greetingMessage: MessageInterface = {
...
export const SysCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        if(!registered) return;
        const chatLogData = await getChat(interaction.channelId);
        let chatLog;
        if (chatLogData) {
            chatLog = assembleChatFromData(chatLogData);
            if(chatLog === null) return;
...
export const toggleVectorCommand: SlashCommand = {
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        const constructs = retrieveConstructs();
        const constructDoc = await getConstruct(constructs[0]);
        const construct = assembleConstructFromData(constructDoc);
        if(construct === null) return;
        const registeredChannels = getRegisteredChannels();
        let registered = false;
